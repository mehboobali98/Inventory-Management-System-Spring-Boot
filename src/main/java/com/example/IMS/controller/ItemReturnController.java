...
public class ItemReturnController {
	...
	public String Create(@Valid @ModelAttribute("itemReturnDto") ItemReturnDto itemReturnDto, BindingResult result) {
		...
		Borrower borrower = borrowerService.getBorrowerById(itemReturnDto.getBorrowerId());
		Item item = itemService.getItemById(itemReturnDto.getItemId());
		Loan loan = itemIssuanceService.findItemIssued(borrower.getId(), item.getId());
		try {
